...
use crate::preprocessing::tokenizer::base_tokenizer::{split_on_special_tokens, tokenize_cjk_chars, whitespace_tokenize};
...
pub fn tokenize_bert(text: &str, vocab: &impl Vocab) -> Vec<String> {
    let tokenized_text: Vec<String> = {
        ...
        let temp_text: Vec<String> = temp_text.iter().map(|v| tokenize_cjk_chars(v)).collect();
        temp_text
    };
